function [o_data,o_valid,o_cnt,o_frame,o_state] = genframe2Sample(Len, Fs,f1,f2,mode,i_ready,rst)%#codegen
...
if(isempty(p_state))
   ...
   switch(p_state)
       case ST_IDLE
                   p_state(:) = ST_SEND;
                   p_cnt(:) = 1;
                   [re,~] = genStim(double(Len), double(Fs),double(f1),double(f2),mode);
